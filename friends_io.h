 ...
 * @param buf ここに必要なメモリを確保して文字列をフォーマットするのです。
 * @param err NULL でない値をよこしてくれたなら、ここにエラーの情報を書き込むのです。
 * @param format フォーマットする文字列をよこすのです。
 * @param ... フォーマットするデータをよこすのです。
 * @return 書き込んだ配列要素数を返すのです。
 *
 * 基本的には C 言語の printf 関数と互換性があるのです。違うところは以下の通りなのです。
 *
 * - 浮動小数点数の変換は（まだ）対応していないのです。
 * - C 言語の文字列型を必要とする箇所はすべて `friendsUnescapeStringLiteral`
 *   を通してフィルタするのです（フォーマット、引数両方）。
 * - `%lc` は Unicode の文字を埋め込めるのです。`int` 型の変数や値をよこすのです。
 * - `%ls` は `friendsChar` の文字列を埋め込むのです。
 * - 文字列の幅と精度はすべて「文字数」でカウントするのです。
 * - `%n` は対応しないのです。
 * - `%#d` （10進の整数変換に `#` 指示）は漢数字というもので印字するのです。
 *   この時の幅と精度は文字列出力に対する扱いと同等になるのです。
 * - フォーマット結果は機種依存なので、システムの printf 関数とは違う結果になる場合
 *   があるのです。我々は BSD の printf 関数の実装を参考にしたのです。
 */
